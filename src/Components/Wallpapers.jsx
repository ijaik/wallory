...
//   const handleDownload = useCallback(async (photoId, photoDescription) => {
//     try {
//       const downloadUrl = `https://api.unsplash.com/photos/${photoId}/download?client_id=${API_KEY}`;
//       const response = await fetch(downloadUrl);
//       if (!response.ok) throw new Error(`Failed to initiate download: ${response.statusText}`);
//       const result = await response.json();
//       const actualDownloadUrl = result.url;
//       const link = document.createElement("a");
